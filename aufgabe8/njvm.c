...
int objRefToInt(ObjRef objRef){
    int value=*(int *)objRef->data;
    return value;
}
...
void new(int x){
    ObjRef record=newCompoundObject(x);
    for(int i=0;i<x;i++){
...
void getf(int x){
    ObjRef record=popObjRef();
    pushObjRef2(GET_REFS_PTR(record)[x]);
}
void putf(int x){
    ObjRef value=popObjRef();
    ObjRef record=popObjRef();
    GET_REFS_PTR(record)[x]=value;
}
void newa(void){
    bip.op1=popObjRef();
   int nb=bigToInt();
   ObjRef array= newCompoundObject(nb);
   for(int i = 0; i<nb; i++){
...
void getfa(void){
 ...
 ObjRef value=GET_REFS_PTR(array)[x];
 pushObjRef2(value);
...
void refeq(void){
    ObjRef obj1=popObjRef();
    ObjRef obj2=popObjRef();
    int res=0;
    if(obj1==obj2){
        res=1;
    }
...
/* gcc -g -Wall -std=c99 -pedantic -I./bigint/build/include -L./bigint/build/lib njvm.c  -lbigint -o ange*/